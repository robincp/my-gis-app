...
// Component to adjust the map bounds based on the markers
function FitBounds() {
  const map = useMap();
  const bounds = nationalGridLocations.map(loc => loc.position);
  map.fitBounds(bounds); // Fit the bounds to include all markers
  return null;
}
